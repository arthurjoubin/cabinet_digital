...
class Command(BaseCommand):
    help = 'Met à jour les données des intégrateurs avec les nouvelles informations'

    def handle(self, *args, **options):
        # Données des intégrateurs du JSON
        integrators_data = {
            "integrateurs": [
                {
                    "nom": "1Life",
        ...
        self.stdout.write(self.style.SUCCESS('Début de la mise à jour des intégrateurs...'))

        updated_count = 0
        created_count = 0

        for integrator_data in integrators_data["integrateurs"]:
            slug = slugify(integrator_data["nom"])
            
            try:
                # Essayer de trouver l'intégrateur existant
                integrator = Integrator.objects.get(slug=slug)
                
                # Mettre à jour les champs
                integrator.description = integrator_data["description"]
                integrator.excerpt = integrator_data["excerpt"]
                integrator.tags = ", ".join(integrator_data["tags"])
                integrator.save()
                
                updated_count += 1
                self.stdout.write(f'✓ Mis à jour: {integrator_data["nom"]}')
                
            except Integrator.DoesNotExist:
                # Créer un nouvel intégrateur si il n'existe pas
                integrator = Integrator.objects.create(
                    name=integrator_data["nom"],
                    slug=slug,
                    description=integrator_data["description"],
                    excerpt=integrator_data["excerpt"],
                    tags=", ".join(integrator_data["tags"]),
                    is_published=True
                )
                
                created_count += 1
                self.stdout.write(f'✓ Créé: {integrator_data["nom"]}')

        self.stdout.write(
            self.style.SUCCESS(
                f'\nTerminé! {updated_count} intégrateurs mis à jour, {created_count} intégrateurs créés.'
            )
        ) 